...
function FormAvaliacao({ concelho }) {
  ...
  const submit = async () => {
    ...
    try {
      await fetch('/api/contact', {
        ...
        body: JSON.stringify({ nome: form.nome, telefone: form.telefone, email: form.email, assunto: 'Avaliação Gratuita – ' + concelho, mensagem: form.morada + (form.mensagem ? ' | ' + form.mensagem : '') })
      });
  ...
  if (enviado) return (
    <div style={{background:'rgba(34,197,94,.1)',border:'1px solid rgba(34,197,94,.3)',borderRadius:14,padding:24,textAlign:'center',color:'#4ade80'}}>
      ✅ Pedido enviado! Marco irá contactá-lo em menos de 24 horas.
    </div>
  );
  ...
  return (
    <div style={{background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.08)',borderRadius:20,padding:30}}>
      ...
      <input style={inp} name="email" placeholder="email@exemplo.com" value={form.email} onChange={handle} />
      <label style={lbl}>Morada do Imóvel</label>
      <input style={inp} name="morada" placeholder={`Rua, nº, ${concelho}`} value={form.morada} onChange={handle} />
      <label style={lbl}>Tipo de Imóvel</label>
      <select style={{...inp, marginBottom:14}} name="tipo" value={form.tipo} onChange={handle}>
        <option>Apartamento</option><option>Moradia</option><option>Terreno</option><option>Comercial</option>
      </select>
      <label style={lbl}>Notas adicionais</label>
      <textarea style={{...inp, minHeight:80, resize:'vertical'}} name="mensagem" placeholder="Informação adicional que queira partilhar..." value={form.mensagem} onChange={handle} />
      <button onClick={submit} disabled={enviando}
        style={{width:'100%',padding:15,background: enviando ? '#555' : '#cc1f1f',color:'#fff',border:'none',borderRadius:10,fontFamily:'Montserrat,sans-serif',fontSize:13,fontWeight:700,cursor: enviando ? 'not-allowed' : 'pointer',letterSpacing:1}}>
        {enviando ? 'A enviar...' : '📋 Solicitar Avaliação Gratuita'}
      </button>
    </div>
  );
}
    ...
    <div style={{background:'#08142a',minHeight:'100vh',fontFamily:'Montserrat,sans-serif',color:'#fff'}}>
      ...
      <div style={{background:'linear-gradient(135deg,#08142a,#1a3670)',padding:'60px 20px 50px',textAlign:'center'}}>
        ...
        <h1 style={{fontSize:'clamp(24px,5vw,42px)',fontWeight:800,marginBottom:16}}>
          Especialista em <span style={{color:'#cc1f1f'}}>Loures</span>
        </h1>
        <p style={{fontSize:15,color:'#7a8daa',maxWidth:560,margin:'0 auto 28px',lineHeight:1.7}}>
          Avaliação gratuita do seu imóvel em Loures. Resposta em menos de 24 horas.
        </p>
        <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
          <a href="https://wa.me/351969692793?text=Olá Marco, quero saber mais sobre imóveis em Loures"
            target="_blank" rel="noreferrer"
            style={{background:'#25D366',color:'#fff',padding:'13px 24px',borderRadius:50,fontWeight:700,fontSize:13,textDecoration:'none',display:'flex',alignItems:'center',gap:8}}>
          ...
          <a href="tel:+351969692793"
            style={{background:'rgba(255,255,255,.12)',color:'#fff',padding:'13px 24px',borderRadius:50,fontWeight:700,fontSize:13,textDecoration:'none',border:'1px solid rgba(255,255,255,.2)',display:'flex',alignItems:'center',gap:8}}>
            📞 969 692 793
          </a>
        </div>
      </div>
      ...
      <div style={{maxWidth:720,margin:'0 auto',padding:'60px 20px'}}>
        ...
        <h2 style={{fontSize:'clamp(20px,4vw,30px)',fontWeight:800,marginBottom:16}}>Imobiliário em <span style={{color:'#cc1f1f'}}>Loures</span></h2>
        <div style={{width:48,height:3,background:'linear-gradient(to right,#cc1f1f,transparent)',borderRadius:2,marginBottom:28}}/>
        <p style={{color:'rgba(255,255,255,.75)',lineHeight:1.8,marginBottom:16,fontSize:15}}>O concelho de Loures é um dos mercados imobiliários mais dinâmicos da Área Metropolitana de Lisboa. Com uma localização privilegiada — a norte da capital, com excelentes acessos pela A8 e A1 — Loures atrai cada vez mais famílias que procuram espaço, qualidade de vida e preços competitivos face a Lisboa.</p>
        <p style={{color:'rgba(255,255,255,.75)',lineHeight:1.8,marginBottom:16,fontSize:15}}>As freguesias de Sacavém, Prior Velho, Moscavide e Portela são as mais procuradas. Em 2026, os apartamentos atingem em média 4.400€/m² e as moradias 3.438€/m², refletindo a forte valorização registada nos últimos anos.</p>

        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:14,margin:'28px 0'}}>
          {[['4.400€/m²','Apartamentos'],['3.438€/m²','Moradias'],['45-75 dias','Tempo médio venda']].map(([num,label])=>(
        ...
        <h2 style={{fontSize:'clamp(20px,4vw,30px)',fontWeight:800,marginBottom:16}}>Serviços em <span style={{color:'#cc1f1f'}}>Loures</span></h2>
        <div style={{width:48,height:3,background:'linear-gradient(to right,#cc1f1f,transparent)',borderRadius:2,marginBottom:28}}/>
        <ul style={{color:'rgba(255,255,255,.75)',lineHeight:1.8,paddingLeft:20,marginBottom:32,fontSize:15}}>
          <li style={{marginBottom:10}}><strong>Avaliação gratuita do imóvel</strong> — Análise comparativa com dados reais de transações recentes em Loures</li>
          ...
          <li style={{marginBottom:10}}><strong>Prospeção de compradores</strong> — Base de dados ativa de compradores à procura de imóveis em Loures</li>
        ...
        <h2 style={{fontSize:'clamp(20px,4vw,30px)',fontWeight:800,marginBottom:16}}>Quanto vale o seu imóvel em <span style={{color:'#cc1f1f'}}>Loures</span>?</h2>
        <div style={{width:48,height:3,background:'linear-gradient(to right,#cc1f1f,transparent)',borderRadius:2,marginBottom:28}}/>
        <FormAvaliacao concelho="Loures" />
      </div>
